...
<form action="<?php echo url_for('adm_ma_emp/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php endif; ?>
  <table class="table table-bordered formulario-documento sin-lineas-border">
    ...
    <tbody>
      ...
      <tr> 
        <th class="linea-left"><?php echo $form['id_principal']->renderLabel() ?></th>
        <td>
          <?php echo $form['id_principal'] ?>
          <?php echo $form['id_principal']->renderError() ?>
        </td>
      </tr>
    </tbody>
  </table>
