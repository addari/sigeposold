...
<form action="<?php echo url_for('adm_ma_tax/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php endif; ?>
  <table class="table table-bordered formulario-documento sin-lineas-border">
    ...
    <tbody>
      ...
      <tr> 
        <th class="linea-left"><?php echo $form['alias']->renderLabel() ?></th>
        <td>
          <?php echo $form['alias'] ?>
      ...
      <tr> 
        <th class="linea-left"><?php echo $form['porcentaje']->renderLabel() ?></th>
        <td>
