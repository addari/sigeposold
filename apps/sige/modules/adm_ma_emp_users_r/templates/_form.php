...
<form action="<?php echo url_for('adm_ma_emp_users_r/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php endif; ?>
  <table class="table table-bordered formulario-documento sin-lineas-border">
    ...
    <tbody>
      ...
      <tr> 
        <th class="linea-left"><?php echo $form['id_empresa']->renderLabel() ?></th>
        <td>
          <?php echo $form['id_empresa'] ?>
          <?php echo $form['id_empresa']->renderError() ?>
        </td>
      </tr>
      <tr> 
        <th class="linea-left"><?php echo $form['id_usuario']->renderLabel() ?></th>
        <td>
